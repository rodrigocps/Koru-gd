    ...
    fetch(`/api/usuarios/avaliacoes${userId? "?userId="+userId : ""}`)
        .then(response => {
            ...
            if(data.length > 0) {
                const sorted = {}
                data.forEach(aval => {
                    const empresaId = aval.empresa.id.toString()
                    if(!Object.keys(sorted).includes(empresaId)){
                        sorted[empresaId] = {...aval.empresa, avaliacoes:[]}
                    }
                    sorted[empresaId].avaliacoes = [...sorted[empresaId].avaliacoes, aval].sort(compararPorId)
                })

                Object.keys(sorted).forEach(empresaId => {
                    const empresa = sorted[empresaId]
                    const div = document.createElement("section")
                    div.id = "empresa:" + empresaId
                    div.innerHTML = `
                        <div id="aval-empresa-header">
                            <img src=${empresa.logo_url} alt="logo-empresa"/>
                            <h3><a href="/empresas?id=${empresa.id}">${empresa.nome}</a></h3><hr/>
                        </div>
                    `
                    
                    empresa.avaliacoes.forEach(avaliacao => {
                        if(avaliacao.titulo && avaliacao.texto && avaliacao.texto && avaliacao.author) {
                            const li = document.createElement("li");
                            li.className = "avaliacao-li"
                            renderAvaliacao(li, avaliacao)
...
function getEdicaoBlock(parent,savedAvaliacao) {
    ...
    edicaoBlock.appendChild(renderEditarAvaliacaoButtons(savedAvaliacao, tituloInput, textInput))

    return edicaoBlock
}

function renderEditarAvaliacaoButtons(savedAvaliacao, tituloInput, textInput) {
    ...
    saveButton.addEventListener("click", () => {
        ...
        if((avaliacao.titulo!==savedAvaliacao.titulo) || (avaliacao.texto!==savedAvaliacao.texto)) {
            fetch(`/api/empresas/${savedAvaliacao?.empresa_id??savedAvaliacao.empresa.id}/avaliacoes/${savedAvaliacao.id}`, {
                method: 'PUT',
                headers: {
                  'Content-Type': 'application/json',
                },
...
function renderButtonsDiv(parent, avaliacao, isClientAdmin) {
    ...
    if(user) {
        ...
        deleteButton.addEventListener("click", () => deletarAvaliacao(avaliacao?.empresa_id??avaliacao.empresa.id, avaliacao.id))
